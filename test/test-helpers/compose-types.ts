...
export type Middleware<T = undefined> = (next: T extends undefined ? (() => Promise<void>) : (arg0: T) => Promise<void>) => Promise<void>;
...
type GetReturnTypesFromMiddlewares<T extends Array<any>> = T extends [infer u, ...infer v] ?
  [u extends Middleware<infer r> ? r : never, ...GetReturnTypesFromMiddlewares<v>] : [];

/** The actual test handler used to execute a set of Tape tests. */
export type TestHandler<T extends any[]> = (...arg: [...RemoveGenericFromTuple<GetReturnTypesFromMiddlewares<T>, undefined>, tape.Test]) => Promise<void> | void;
